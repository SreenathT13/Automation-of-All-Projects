while 1:
    choice = int(input('This is CN274: \n'
                       '1: Full_function:\n'
                       '2: Exit\n'
                       'Option: \n'))
    import time
    ...
    def header_function():
        ...
        if "Ready" in connectText.text:
            ...
            CF.take_image(pdf, CS.old_live_video_xpath,
                          'D:\\TenXer\\gmail_login\\CN274\\screenshot\\live_image.png',
                          'live_image.png')
    ...
    if choice == 1:
        def full_function():
            ...
            CF.wait.until(CF.EC.text_to_be_present_in_element((By.XPATH, CS.old_connection_path), 'Ready'))
            hover = CF.driver.find_element(By.XPATH, CS.old_live_video_xpath)
            CF.actions.move_to_element(hover).perform()
            CF.driver.switch_to.frame(CF.driver.find_element(By.XPATH, CS.old_switch_to_frame))
            ...
            CF.take_image(pdf, CS.old_live_video_xpath,
                          'D:\\TenXer\\gmail_login\\CN274\\screenshot\\Full_live_image.png',
                          'Full_live_image.png')
            time.sleep(5)
            CF.click_button(CN.maximize_graph)
            time.sleep(5)
            CF.take_image(pdf, CN.graph_path, 'D:\\TenXer\\gmail_login\\CN274\\screenshot\\graph.png',
                          'graph.png')
            CF.click_button(CN.maximize_graph)
            CF.old_update_progress_log(pdf)
